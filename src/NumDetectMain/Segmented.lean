...
/-- Automatic segmented Vandermonde bound in the exact parity and dimension
range covered by the registered centered-cube source. -/
theorem segmentedVandermonde_minimumSingularValue_of_evenLocalizationOrder
    {d n A nStar m r D : ℕ} {τ η β : ℝ}
    (μ : AtomicMeasure d n)
    (hd : 2 ≤ d) (hn : 2 ≤ n)
    (hclumps : IsAngularClumpStructure μ.node A nStar τ η)
    ...
    (hβ : 1 / (2 * Real.log 2) < β)
    (hη : 4 * Real.pi * β * d / (localizationOrder m nStar + 1) ≤ η)
    (hKeven : Even (localizationOrder m nStar))
    (hr : 2 * nStar ≤ r)
    (hlocal :
      periodicMinimumL1Separation μ.node hn ≤
        Real.pi * nStar / ((r * D : ℕ) : ℝ)) :
  ...
  apply
    segmentedVandermonde_minimumSingularValue_of_colorFrames
      (m₁ := localizationHalfWidth m) (b := m / 2)
      (K := localizationOrder m nStar)
      μ (by omega) hn hclumps hsplit hK hDpos hτ hβ hr hΔ hmin hscale
  intro C anchor _hsame hcross color v
  apply fineCube_frame_of_evenCutoff hd hKeven β η hβ hη
...
/-- Automatic segmented threshold theorem in dimensions `d ≥ 2` when the
localization cutoff is even. -/
theorem segmentedGHM_singularValueThreshold_of_evenLocalizationOrder
    {d n A nStar m r D : ℕ} {τ η β σ mMin : ℝ}
    (μ : AtomicMeasure d n) (Y : Point d → ℂ)
    (hd : 2 ≤ d) (hn : 2 ≤ n)
    (hclumps : IsAngularClumpStructure μ.node A nStar τ η)
  ...
  · intro hlocal hseparation
    have hB :
        B ≤ matrixSingularValue
          (segmentedVandermonde m r D μ.node) (n - 1) := by
      exact
        segmentedVandermonde_minimumSingularValue_of_evenLocalizationOrder
          μ hd hn hclumps hm hD hτ hβ hη hKeven hr hlocal
    exact
      (segmentedGHM_singularValueThreshold_of_segmentedLowerBound
        μ Y (by omega) hn hclumps hm hD hτ hβ hη hr hσ hmMin hnoise
        hmeasurement (B := B) le_rfl hB).2 hlocal hseparation
